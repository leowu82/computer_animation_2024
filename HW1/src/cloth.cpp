...
void Cloth::initializeSpring() {
  ...
  for (int i = 0; i < particlesPerEdge - 1; i++) {
    for (int j = 0; j < particlesPerEdge - 1; j++) {
      ...
      _springs.emplace_back(index + 1, index + particlesPerEdge, shearLength, Spring::Type::SHEAR);

    }
  }

  // Bending spring
  float bendingLength = (_particles.position(0) - _particles.position(2)).norm();
  for (int i = 0; i < particlesPerEdge; ++i) {
...
void Cloth::computeSpringForce() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    Eigen::Vector4f dist = _particles.position(startIndex) - _particles.position(endIndex);
    float posDiff = (dist).norm();

    // spring force
    Eigen::Vector4f force = -springCoef * (posDiff - cur->length()) * (dist / posDiff);

    // damper force
    Eigen::Vector4f v_dist = _particles.velocity(startIndex) - _particles.velocity(endIndex);
    force -= damperCoef * (v_dist.dot(dist) / posDiff) * (dist / posDiff);

    // change acceleration
    _particles.acceleration(startIndex) += force * _particles.inverseMass(startIndex);
    _particles.acceleration(endIndex) -= force * _particles.inverseMass(endIndex);
  }
}
